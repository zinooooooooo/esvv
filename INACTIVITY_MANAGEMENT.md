...
This document describes the inactivity management system implemented for the eSVMWDO application. The system automatically archives users inactive for 20 days and deletes users inactive for 30+ days.
...
- **Automatic Deletion**: Users inactive for 30+ days are permanently deleted
...
1. **Archives Users (20 days inactive)**:
   - Finds users with `last_active` between 20-30 days ago
   - Sets `archived = TRUE`
   - Logs the action in `user_activity_logs`

2. **Deletes Users (30+ days inactive)**:
   - Finds archived users with `last_active` older than 30 days
...
- Reason (`inactive_20_days`, `inactive_30_days`, etc.)
- Days inactive
...
2. **Inactivity Status**: Color-coded status badges:
   ...
   - Red: Over 30 days (will be deleted)
...
WHERE inactivity_status IN ('over_20_days', 'over_30_days')
...
```sql
-- Change 20 days to 15 days
AND last_active < NOW() - INTERVAL '15 days'

-- Change 30 days to 45 days
AND last_active < NOW() - INTERVAL '45 days'
```
