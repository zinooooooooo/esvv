...
const UserManagement = () => {
  ...
  const getInactivityStatus = (daysInactive, user) => {
    // Staff and admin are exempt from inactivity management
    if (user && (user.role === "staff" || user.role === "admin")) {
      return { label: "Exempt", color: "gray" };
    }
                      ...
                      <tr key={user.id} className={`hover:bg-gray-50 ${showArchived ? 'bg-amber-50' : ''}`}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                          {(() => {
                            ...
                            const status = getInactivityStatus(daysInactive, user);
                            const colorClasses = {
